...
function twomass!(dz, z, p, t)
    k1 = 1.0
    k2 = sqrt(3.0) - p / 2 * sin(t)^2 + p / 4 * sin(t)
    n1 = 0.02
    n2 = 0.02
    n3 = 0
    d1 = 0.03
    ...
    dz .= [z[3];
        z[4];
        -(k1 * z[1]) - k2 * z[1] - 12 * n1 * z[1]^3 - 12 * n2 * z[1]^3 - 360 * n3 * z[1]^5 + k2 * z[2] + 36 * n2 * z[1]^2 * z[2] + 1800 * n3 * z[1]^4 * z[2] - 36 * n2 * z[1] * z[2]^2 - 3600 * n3 * z[1]^3 * z[2]^2 + 12 * n2 * z[2]^3 + 3600 * n3 * z[1]^2 * z[2]^3 - 1800 * n3 * z[1] * z[2]^4 + 360 * n3 * z[2]^5 - d1 * z[3] - d2 * z[3] + d2 * z[4] + p * cos(t);
        k2 * z[1] + 12 * n2 * z[1]^3 + 360 * n3 * z[1]^5 - k2 * z[2] - 36 * n2 * z[1]^2 * z[2] - 1800 * n3 * z[1]^4 * z[2] + 36 * n2 * z[1] * z[2]^2 + 3600 * n3 * z[1]^3 * z[2]^2 - 12 * n2 * z[2]^3 - 3600 * n3 * z[1]^2 * z[2]^3 + 1800 * n3 * z[1] * z[2]^4 - 360 * n3 * z[2]^5 + d2 * z[3] - d2 * z[4] + p * sin(t)]
    return dz
end

function twomass(z, p, t)
    dz = zero(z)
    return twomass!(dz, z, p, t)
end

## Set up some system parameters
Amplitude_list = (0.0, 0.1) # forcing amplitude
omega_ode = 0.69 * 1.1 # Float64(pi) # forcing frequency
...
maxSimAmp = 0.6     # maximum initial condition measured from the torus
maxAmp = 0.6        # filter out data with amplitude greater than maxAmp
...
all_revision = "twomass-2p$(log2(scale_epsilon))-SIMAMP$(maxSimAmp)-F$(maximum(fourier_order_list))-R$(R_order)-U$(U_order)-V$(V_order)-S$(S_order)-MO$(rom_order)-MODE$(SEL[1])"
...
for (Amplitude, fourier_order, surf_lim) in zip(Amplitude_list, fourier_order_list, surf_lim_list)
    ...
    revision = "twomass-2p$(log2(scale_epsilon))-SIMAMP$(maxSimAmp)-AMP$(Amplitude)-F$(fourier_order)-R$(R_order)-U$(U_order)-V$(V_order)-S$(S_order)-MO$(rom_order)-MODE$(SEL[1])"
    datarevision = "twomass-SIMAMP$(maxSimAmp)-AMP$(Amplitude)-F$(fourier_order)"
    ...
    XPode = fromFunction(MPode, (x, t) -> twomass(x, Amplitude, t))
    ...
    if map_manifolds
        ...
        XPmap = zero(MP)
        mapFromODE(MP, XPmap, twomass!, Amplitude, omega_ode, Tstep / 500, Tstep)

        # The invariant manifold of the map is calculated using [`QPMAPTorusManifold`](@ref)
        MK, XK, MSn, XSn, MWn, XWn, MSd, XSd = QPMAPTorusManifold(MP, XPmap, omega, SEL, threshold=0.1, resonance=false, rom_order=rom_order)
        ...
        MRf, XRf, MWf, XWf, MRt, XRt, MUt, XUt, MSt, XSt, MVt, XVt = QPGraphStyleFoliations(MP, XPmap, omega, SEL; dataScale=1, resonance=false, threshold=0.1, rom_order=rom_order)
        # The results are plotted
    ...
    if create_data
        ...
        dataX, dataY, thetaX, thetaY, thetaNIX, thetaNIY = generate(NDIM, twomass!, ones(NDIM) * maxSimAmp, 600, 50, fourier_order, omega_ode, Tstep, Amplitude, XWre)
