...
function carfollow!(dz, z, p, t)
    L = NDIM + 1 # = 2 * n
    v0 = 1
    a = 0.75
    F = p * cos(t)
    dz .= [a * (-(((-5 + L)^2 * v0) / (50 + (-10 + L) * L)) - z[1] + (v0 * (L - 5 * (1 + z[6] + z[7] + z[8] + z[9]))^2) / (25 * (1 + (L - 5 * (1 + z[6] + z[7] + z[8] + z[9]))^2 / 25))),
        a * (-z[2] + 25 * v0 * (1 / (50 + (-10 + L) * L) - 1 / (50 + L^2 + 10 * L * (-1 + z[6]) + 25 * (-2 + z[6]) * z[6]))),
        a * (-z[3] + 25 * v0 * (1 / (50 + (-10 + L) * L) - 1 / (50 + L^2 + 10 * L * (-1 + z[7]) + 25 * (-2 + z[7]) * z[7]))),
        a * (-z[4] + 25 * v0 * (1 / (50 + (-10 + L) * L) - 1 / (50 + L^2 + 10 * L * (-1 + z[8]) + 25 * (-2 + z[8]) * z[8]))),
        a * (-(((-5 + L)^2 * v0) / (50 + (-10 + L) * L)) - z[5] + ((F + v0) * (-1 + L / 5 + z[9])^2) / (1 + (-1 + L / 5 + z[9])^2)),
        z[1] - z[2],
        z[2] - z[3],
        z[3] - z[4],
        z[4] - z[5]]
...
function carfollow(z, p, t)
    dz = zero(z)
    return carfollow!(dz, z, p, t)
end

## Set up some system parameters
Amplitude_list = (0.0, 0.2) # forcing amplitude
fourier_order_list = (0, 7) # fourier harmonics to be resolved
omega_ode = 0.486 * 0.9 # forcing frequency
...
ODE_order = 5 # polynomial order of the calculations
...
maxSimAmp = 1.2     # maximum initial condition measured from the torus
...
S_order = 2         # polynomial order of S
STEPS = 4000         # number of optimisation steps to use
...
xlims!(axFreq, 0.45, 0.52)
ylims!(axFreq, 0, dispMaxAmp)
xlims!(axDamp, 0.03, 0.1)
ylims!(axDamp, 0, dispMaxAmp)

all_revision = "carfollow-2p$(log2(scale_epsilon))-SIMAMP$(maxSimAmp)-R$(R_order)-U$(U_order)-V$(V_order)-S$(S_order)-MODE$(SEL[1])"

figEvArr = []
axTorArr = []
for (Amplitude, fourier_order) in zip(reverse(Amplitude_list), reverse(fourier_order_list))

    # the names of data files
    revision = "carfollow-2p$(log2(scale_epsilon))-SIMAMP$(maxSimAmp)-AMP$(Amplitude)-F$(fourier_order)-R$(R_order)-U$(U_order)-V$(V_order)-S$(S_order)-MODE$(SEL[1])"
    datarevision = "carfollow-SIMAMP$(maxSimAmp)-AMP$(Amplitude)-F$(fourier_order)"
    ...
    XPode = fromFunction(MPode, (x, t) -> carfollow(x, Amplitude, t))
    ...
    if map_manifolds
        ...
        XPmap = zero(MP)
        mapFromODE(MP, XPmap, carfollow!, Amplitude, omega_ode, Tstep / 500, Tstep)
    ...
    if create_data && !load_pre_trained
        ...
        dataX, dataY, thetaX, thetaY, thetaNIX, thetaNIY = generate(NDIM, carfollow!, ones(NDIM) * maxSimAmp, 600, 50, fourier_order, omega_ode, Tstep, Amplitude, XWre)
    ...
    elseif !train_model
        ...
        MCF, XCF = QPCombinedFoliation(NDIM, 2, fourier_order, R_order, U_order, S_order, V_order, R1, S1, MK1, zero(XK1), sparse=true)
        dataIdV[1] = preId
    end
...
#     lines!(axErr, errMeanX .+ errStdX, errMeanY, color=pl_color, linestyle=:dot, linewidth=3)
    ...
    if map_manifolds
        ...
        @save "result-$(revision).bson" odeFreq odeDamp odeAmp dataFreq dataDamp dataAmp OnManifoldAmplitude hsU errMaxX errMaxY errMinX errMinY errMeanX errMeanY errStdX
    end
